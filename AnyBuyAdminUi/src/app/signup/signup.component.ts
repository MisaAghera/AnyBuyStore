...
export class SignupComponent implements OnInit {
  ...
  registerUser = (registerFormValue: any) => {
    ...
    switch(formValues.userType){
      case "seller":{
        this.authService.registerSeller(userForRegister)
        .subscribe({
          next: (_) => {
          ...
          error: (err: HttpErrorResponse) => {
            ...
            document.getElementById("danger-alert")!.innerHTML = "registration was unsuccessfull, please check the details";
          }
        })
      }
      break;
      case "admin":{
        this.authService.registerAdmin(userForRegister)
        .subscribe({
          next: (_) => {
            alert("registration is successfull");
      ...
      break;
      case "customer":{
        this.authService.registerCustomer(userForRegister)
        .subscribe({
          next: (_) => {
            alert("registration is successfull");
            this.router.navigate(["/login"]);
